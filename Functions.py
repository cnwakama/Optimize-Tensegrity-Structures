...
class Functions:
    ...
    def function(self, f):
        func = sy.lambdify([self.xe, self.xs, self.length, self.mass, self.e3], f, "numpy")
        #g = [sy.diff(f, x) for x in [self.xs, self.xs]]
        g = derive_by_array(f, [self.xs, self.xe])
        grad = sy.lambdify([self.xe, self.xs, self.length, self.mass, self.e3], g, "numpy")
        h = derive_by_array(g, [self.xs, self.xe])
        hess = sy.lambdify([self.xe, self.xs, self.length, self.mass, self.e3], h, "numpy")
    ...
    def gravitationalEnergy(self):
        f = self.mass * self.gravity * (1.0/2.0) * self.e3 * (self.xs + self.xe)
        #g = [sy.diff(f, x) for x in [self.xs, self.xs]]
